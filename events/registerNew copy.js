              ...
              if (clan) {
                ...
                db.query(sql1, (err, rows) => {
                  if (rows.length != 0) {
                    ...
                    const sql2 = await `UPDATE registry SET activo=0 WHERE usernameds='${message.author.username + '#' + message.author.discriminator}'`
                    db.query(sql2, (err, rows) => { })
                    const sql1 =  await `SELECT * FROM clanes`
                    db.query(sql1, (err, rows) => {
